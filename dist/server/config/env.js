...
export function loadConfig() {
    ...
    const primaryRpcUrl = process.env.BSC_PRIMARY_RPC_URL ||
    ...
    const fallbackRpcUrls = fallbackRpcsString
        ? fallbackRpcsString.split(',').map((s) => s.trim()).filter(Boolean)
        : [];
    ...
    const mevProtectionValidators = mevProtectionValidatorsString.split(',').map((s) => s.trim());
    const config = {
        // Server
