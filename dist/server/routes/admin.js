 ...
 */
router.get('/api-keys', requireAdminToken, async (_req, res) => {
    try {
...
router.get('/api-keys/:id/limits', requireAdminToken, async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            res.status(400).json({
 ...
 */
router.post('/api-keys', requireAdminToken, async (req, res) => {
    try {
...
router.post('/api-keys/:id/deactivate', requireAdminToken, async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/usage', requireAdminToken, async (req, res) => {
    try {
...
router.get('/usage/recent', requireAdminToken, async (req, res) => {
    try {
...
router.get('/usage/errors', requireAdminToken, async (req, res) => {
    try {
        const { apiKeyId, limit, since } = req.query;
        const parsedLimit = typeof limit === 'string'
            ? Math.min(Math.max(parseInt(limit, 10) || 0, 1), 500)
